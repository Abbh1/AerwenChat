...
namespace ARW.Admin.WebApi.Controllers
{
    ...
    public class TasksController : BaseController
    {
        ...
        public IActionResult Create([FromBody] TasksCreateDto parm)
        {
            ...
            {
                tasksQz.AssemblyName = "ARW.Tasks";
        ...
        {
            //判断是否已经存在
            if (_tasksQzService.Any(m => m.Name == parm.Name && m.ID.ToString().ToString() != parm.ID.ToString()))
            {
            ...
            var tasksQz = _tasksQzService.GetFirst(m => m.ID.ToString().ToString() == parm.ID.ToString());
            if (string.IsNullOrEmpty(parm.ApiUrl) && parm.TaskType == 2)
            {
            ...
            var response = _tasksQzService.Update(m => m.ID.ToString().ToString() == parm.ID.ToString(), m => new SysTasksQz
            {
                Name = parm.Name,
        ...
        public async Task<IActionResult> Delete(string id)
        {
            ...
            if (!_tasksQzService.Any(m => m.ID.ToString().ToString() == id))
            {
                throw new CustomException("任务不存在");
            }

            var tasksQz = _tasksQzService.GetFirst(m => m.ID.ToString().ToString() == id);
        ...
        public async Task<IActionResult> Stop(string id)
        {
            ...
            if (!_tasksQzService.Any(m => m.ID.ToString() == id))
            {
                throw new CustomException("任务不存在");
            }

            var tasksQz = _tasksQzService.GetFirst(m => m.ID.ToString() == id);
